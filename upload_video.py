...
def get_authenticated_service(args):
  credentials = google.oauth2.credentials.Credentials(args.accessToken)
...
if __name__ == '__main__':
  ...
  argparser.add_argument("--accessToken", required=True, help="Token Google API")
...
async def upload_video(
  ...
  description: str = Form(),
    ...
    insert_request = youtube.videos().insert(
        ...
        # running on App Engine, you should set the chunksize to something like
        # 1024 * 1024 (1 megabyte).
        media_body=http.MediaFileUpload(media, chunksize=-1, resumable=True)
    )
    return
